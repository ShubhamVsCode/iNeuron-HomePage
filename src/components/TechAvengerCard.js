...
const TechAvengerCard = ({ image, name, post, className }) => {
  return (
    ...
    >
      ...
      />
      <img
        className="absolute top-0 w-56 duration-500 ease-in-out bg-purple-100 rounded-full shadow-md group-hover:-top-10 group-hover:shadow-2xl group-hover:bg-purple-300 group-hover:shadow-purple-500 shadow-purple-400 "
        src={image}
        alt=""
      />
      <h1 className="absolute bottom-0 text-2xl font-semibold text-center duration-300 opacity-0 text-ineuron group-hover:opacity-100 group-hover:translate-y-2">
        {name}
      </h1>
